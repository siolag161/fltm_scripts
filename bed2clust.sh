...
R_command="bim <- read.table(\"${ped_path}.bim\", as.is = T, header = F); chromosome = as.numeric(tail(names(sort(table(bim[,1]))), 1)); idx = which(bim[,1] == chromosome) + 6; raw_ped <- read.table(\"${out_path}.raw\", as.is = TRUE, header = T); raw_ped = raw_ped[, idx]; idx = idx - 6; bim = bim[idx, c(1,2,4)]; bim = cbind( idx, bim ); label = bim[, c(2,1,3,4)]; colnames(label) = c(\"chr\", \"idx\", \"label\", \"position\"); write.table( t(raw_ped), file=\"${out_path}_data.csv\", row.names=F, col.names=F, sep=\",\",na=\"-1\"); write.table( label, file=\"${out_path}_label.csv\", row.names=F, col.names=F, sep=\",\");";
